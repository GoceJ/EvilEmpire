    ...
    public function run()
    {
        ...
        foreach ($players as $value) {
            try {
                ...
                file_put_contents('../database/data/logs/basketball_players_error.txt', PHP_EOL . '~~~' .  now() . "~~~" . PHP_EOL . $th->getMessage() . PHP_EOL . json_encode($value) . PHP_EOL, FILE_APPEND);
            }
        }

        file_put_contents('../database/data/seeded_data/basketball_players.json', $json . ',', FILE_APPEND);
        file_put_contents('../database/data/basketball_players.json', '');
        file_put_contents('../database/data/logs/basketball_players_seeded.txt', now() . PHP_EOL, FILE_APPEND);
    }
}
